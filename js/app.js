...
const DEMO_KEY = 'paper-cuts-demo-artworks-v3';
...
async function handleSubmit(event) {
  ...
  try {
    ...
    if (isFirebaseMode) {
      ...
      const storagePath = `artworks/${uid}/${Date.now()}-${slugify(title)}.png`;
...
function listenToFirebaseGallery() {
  ...
  onSnapshot(
    artworksQuery,
    (snapshot) => {
      const artworks = snapshot.docs.map((docSnap) => ({ id: docSnap.id, ...docSnap.data() }));
...
function renderGallery(artworks) {
  ...
  artworks.forEach((artwork) => {
    ...
    deleteBtn.dataset.id = artwork.id;
    deleteBtn.dataset.title = artwork.title || 'this artwork';
    deleteBtn.dataset.storagePath = artwork.storagePath || '';
    deleteBtn.dataset.imageUrl = artwork.imageUrl || '';
    deleteBtn.dataset.mode = isFirebaseMode ? 'firebase' : 'local';
    deleteBtn.disabled = deleteInProgressId === artwork.id;
    if (deleteInProgressId === artwork.id) {
...
async function handleGalleryClick(event) {
  ...
  const artworkTitle = deleteBtn.dataset.title || 'this artwork';
  const mode = deleteBtn.dataset.mode;
  const storagePath = deleteBtn.dataset.storagePath;
  const imageUrl = deleteBtn.dataset.imageUrl;

  const confirmed = window.confirm(`Delete "${artworkTitle}"?`);
  if (!confirmed) return;

  deleteInProgressId = artworkId;
  renderRefreshCurrentGallery();
    ...
    if (mode === 'firebase') {
      ...
      if (storagePath) {
        ...
        try {
          ...
          console.warn('Could not delete storage file by URL.', storageError);
        }
      }
      await deleteDoc(doc(db, 'artworks', artworkId));
      showStatus('Artwork deleted from the shared gallery.');
    } else {
      deleteLocalArtwork(artworkId);
      loadLocalGallery();
      showStatus('Artwork deleted from local demo mode.');
    }
  } catch (error) {
    console.error(error);
...
function renderRefreshCurrentGallery() {
  if (isFirebaseMode) {
    return;
  }
  loadLocalGallery();
...
function deleteLocalArtwork(artworkId) {
  const list = JSON.parse(localStorage.getItem(DEMO_KEY) || '[]');
  const next = list.filter((item) => item.id !== artworkId);
